...
class ActorNetwork(object):
    ...
    def create_actor_network(self):
        ...
        # FC2
        with tf.name_scope("FC2"):
            w_fc2 = weight_variable_uniform([n_hidden_1, n_hidden_2], tf.sqrt(1.0/(n_hidden_1)))
            b_fc2 = bias_variable_uniform([n_hidden_2], tf.sqrt(1.0/(n_hidden_2)))
        # Out_0
        with tf.name_scope("Out"):
            w_out_0 = weight_variable_uniform([n_hidden_2, self.a_dim/2], 3e-3)
            b_out_0 = bias_variable_uniform([self.a_dim/2], 3e-3)
        # Out_1
        with tf.name_scope("Out"):
            w_out_1 = weight_variable_uniform([n_hidden_2, self.a_dim/2], 3e-3)
            b_out_1 = bias_variable_uniform([self.a_dim/2], 3e-3)

        h_fc1 = tf.nn.relu(tf.matmul(inputs, w_fc1) + b_fc1)
        h_fc2 = tf.nn.relu(tf.matmul(h_fc1, w_fc2) + b_fc2)

        # Run sigmoid on output 0 to get 0 to 1
        out_0 = tf.nn.sigmoid(tf.matmul(h_fc2, w_out_0) + b_out_0)
        # Run tanh on output 1 to get -1 to 1
        out_1 = tf.nn.tanh(tf.matmul(h_fc2, w_out_1) + b_out_1)
