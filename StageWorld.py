...
class StageWorld():
	...
	def GetRewardAndTerminate(self, t, switch_a=None):
		terminate = False
		reset = False
		...
		metric = 0.
		result = 0
		if v == 0.0 and laser_min < 0.4 / 5.6 - 0.5:
			self.stop_counter += 1
		else:
			self.stop_counter = 0
		...
		return reward, terminate, result, metric
	...
	def OAController(self, action_bound, last_action):
		scan = (self.GetLaserObservation() + 0.5) * 5.6 - 0.19
		beam_num = len(scan)
