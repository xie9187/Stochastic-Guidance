...
DELTA = 0.3 # The rate of change (time)
SIGMA = 0.3 # Volatility of the stochastic processes
OU_A = 1. # The rate of mean reversion
...
SWITCH_DECAY_START = 400000
SWITCH_DECAY_END = 500000
RANDOM_SWITCH = False
...
def train(sess, env, actor, critic, noise, reward, discrete, action_bound):
    ...
    for i in range(MAX_EPISODES):
        ...
        while not terminal and not rospy.is_shutdown():
            ...
            ep_metric += metric
            if terminal:
                ep_r_window.append(ep_reward)

            if j > 0 :
                memory.store_transition(state, a[0], r, terminal, state1)      #Add replay buffer
            ...
            if T > BUFFER_SIZE+5000 or LOAD_FLAG:
                ...
                # # switch
                # if terminal and T >= DELAY_SWITCH_TRAINING and T < SWITCH_DECAY_END:
                #     if MVG_FLAG:
                #         baseline_MAV = np.mean(ep_r_window)
                #     else:
                #         baseline_MAV = 0.
                #     if not RANDOM_SWITCH:
                #         _ = critic.switch_train(ep_state, ep_switch, ep_reward - baseline_MAV)
